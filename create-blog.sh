...
# ============================================================
# ADICIONAR BOTÃO "BLOG" UMA ÚNICA VEZ (idempotente)
# ============================================================
echo "🔘 Adicionando botão 'Blog' na barra superior..."
...
find book -maxdepth 1 -name "*.html" ! -name "print.html" ! -name "blog.html" -type f | while read -r page; do
    # Remove qualquer instância de botão Blog já existente (para evitar duplicação)
    sed -i '/href="blog\.html".*Blog<\/a>/d' "$page"
    
    # Agora insere dentro de .right-buttons
...
echo "✅ Botão 'Blog' único adicionado em todas as páginas."